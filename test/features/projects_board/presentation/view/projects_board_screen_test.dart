...
class _FailRepo implements ProjectsBoardRepository {
  @override
  Future<Result<ProjectBoardData>> fetchBoard() async => Result.failure('load failed', StackTrace.current);
}
...
void main() {
  ...
  testWidgets('error from board provider shows message + Retry', (tester) async {
    await tester.pumpWidget(
      _app(
        overrides: [
          selectedProjectProvider.overrideWith(() => _SelStub(const ProjectRef(owner: 'o', number: 4, title: 'B'))),
